...
% varargin{4}: determine n-sigma rule.
%varargin{5},varargin{6}: [x x x x x x x x] eight binary numbers that sign the working of eight sub-filters, the first four
%numbers denote the real-part, the last four denote imaginary-part

if strcmp(Type,'3D')
    %for iScale_X = 1: length(EnvelopScale(:,1))
    ScaleTemp = varargin{1}; % get the scales of all the directions
    
    %%%%harmonic construction
    FreqMagnitude = varargin{2};
    thetaFreq = varargin{3}(1);
    phiFreq = varargin{3}(2);
    ...
    [GaborKerCosX,GaborKerCosY,GaborKerCosZ,GaborKerSinX,GaborKerSinY,GaborKerSinZ]=...
        fComputeGaborKernelV3sep(ScaleTemp, [Freq_X, Freq_Y, Freq_Z],varargin{4});
    
    %% Real part
    CubeRe = zeros(size(Cube));
    for IdxRe = 1:length(varargin{5})
        if IdxRe ==1
            if varargin{5}(1) ==1
